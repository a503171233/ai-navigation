...
if ($logRetention > 0) {
    ...
    $db = dcai_db();
    try {
        $v = $db->execute('DELETE FROM verify_logs WHERE created_at < ?', [$cutoff]);
        $m = $db->execute('DELETE FROM module_invoke_logs WHERE created_at < ?', [$cutoff]);
        $u = $db->execute('DELETE FROM update_apply_logs WHERE created_at < ?', [$cutoff]);
        $ops = $db->execute('DELETE FROM operation_logs WHERE created_at < ?', [$cutoff]);
        $hist = $db->execute('DELETE FROM instance_commands WHERE executed_at IS NOT NULL AND executed_at < ?', [$cutoff]);
